...
# Source/read stuff (download data, figures, analysis)
...
```{r echo=FALSE, out.width='60%', fig.cap= "Fig caption text here"}
...
## Data collection
```{r showDataCollection, echo = FALSE}
...
To download the clean database use: `XXX.R`
To load the community data start with the R script: `XXX.R`
...
The data was then checked visually to detect apparent measurement errors. Unrealistic values were removed. For the trait data this included leaves where leaf dry matter values higher than 1 g/g and leaves with specific leaf area values greater than 500 cm^2^ /g.

This still needs to be checked!!!
and leaf nitrogen values higher than 6.4%. The nitrogen cutoff value was chosen based on the highest published leaf nitrogen values found in the Botanical Information and Ecology Network (Enquist et al., 2009) for the genera in our study.
...
Elevation gradient (n = 36): 
		Transect 1: A, B, C, D, F, G
		Transect 2: A, B, C, D, F, G
		Transect 3: A, B, D, F, G
		Transect 4: A, B, D, F, G
		Transect 5: A, B, D, F, G
		Transect 6: A, B, D, F, G
		Transect 7: A, B, C, D

Nutrient gradient (n = 31):
		Transect 1: A, B, C, D, F, G
    Transect 2: A, B, C, E, F, G 
		Transect 3: A, B, C, E, F, G
		Transect 4: A, B, C, D, F, G
